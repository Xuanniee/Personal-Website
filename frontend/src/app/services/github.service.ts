...
export class GithubService {
  ...
  private readonly colorsUrl = environment.githubColorsUrl;
  ...
  //       Authorization: `Bearer ${environment.githubAccessToken}`,
  //     });
  //   }
  ...
  /**
   * Retrieves a specific individual Github Repository
   *
  ...
  getSpecificRepositoryData(
    ...
    const GET_SPECIFIC_REPO_URL = `${this.backendUrl}/github/${repositoryName}`;
    const cacheKey = `${this.cacheKey}-${repositoryName}`;
    ...
    // Make the API call
    return this.http.get<any>(GET_SPECIFIC_REPO_URL).pipe(
      map((data) => {
  ...
  getPublicRepoList(username: string): Observable<any[]> {
    ...
    const ALL_PUBLIC_REPO_URL = `${this.backendUrl}/github`;

    // Make the API call and filter the results
    return this.http.get<any[]>(ALL_PUBLIC_REPO_URL).pipe(
      map((repos) => {
